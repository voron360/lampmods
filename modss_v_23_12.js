...
(function () {
	...
	var TRASH_R = ['$$$####!!!!!!!', '^^^^^^##@', '@!^^!@#@@$$$$$', '^^#@@!!@#!$', '@#!@@@##$$@@'];
	var version_modss = '3.0', API = Protocol() + 'api.lampa.stream/', type = '', jackets = {}, cards, ping_auth, manifest, menu_list = [], vip = true, leftVipD = ' 💎 91 день', user_id = 5897013793, uid = '87375:60efb6412cd5083cf154c58d043d104a_5897013793', IP, logged = true;
    ...
    auth: function () {
		...
		balansers: function() {
		  var balansers = {"videocdn":"VideoCDN","collaps":"Collaps","kinobase":"Kinobase","filmix":"Filmix","cdnmovies":"CDNmovies","rezka":"Rezka","hdr":"HDR MODS's [4K]  <span style=\"font-weight: 700;color:rgb(236,151,31)\">VIP</span>","hdrezka":"HDRezka 4K  <span style=\"font-weight: 700;color:rgb(236,151,31)\">VIP</span>","videodb":"VideoDB  <span style=\"font-weight: 700;color:rgb(236,151,31)\">VIP</span>","zetflix":"Zetflix  <span style=\"font-weight: 700;color:rgb(236,151,31)\">VIP</span>","uakino":"UAKino  <span style=\"font-weight: 700;color:rgb(236,151,31)\">VIP</span>","eneida":"Eneida(Ukr)  <span style=\"font-weight: 700;color:rgb(236,151,31)\">VIP</span>","kodik":"Kodik  <span style=\"font-weight: 700;color:rgb(236,151,31)\">VIP</span>","anilibria":"Anilibria  <span style=\"font-weight: 700;color:rgb(236,151,31)\">VIP</span>","hdvb":"HDVB  <span style=\"font-weight: 700;color:rgb(236,151,31)\">VIP</span>","kinotochka":"KinoTochka  <span style=\"font-weight: 700;color:rgb(236,151,31)\">VIP</span>","kinofit":"KinoFit 4K  <span style=\"font-weight: 700;color:rgb(236,151,31)\">VIP</span>"};
    ...
    speedTest: function(url, params) {
      ...
      var timeStart = false, pl = $('#pl', tpl), m3u = false, testStart = false;
      var pl_p = [[-250, -250]];
      var tout;
      ...
      function setSpeed(v) {
        ...
        pl.attr('points', pl_p.map(function(pt){return pt.join(',')}).join(' '));
      }
  ...
  function videocdn(component, _object) {
    ...
    this.search = function (_object,data) {
      ...
      var url = Protocol() +'videocdn.tv/api/';
    ...
    function extractData(results) {
      ...
      if (movie) {
        ...
        var call_fail = function call_fail(data) {
          get_links_wait = false;
        ...
        if (iframe_proxy) {
          component.proxyCall('GET', src, 20000, null, call_success, call_fail);
            ...
            network.clear();
            network.timeout(20000);
            network.silent('https://cors.lampa.stream/' + src, call_success, call_fail, false, {
              dataType: 'text'
            });
        } else {
  ...
  function component(object) {
    ...
    this.proxy = function (name) {
  		...
  		var proxys = 'https://cors.lampa.stream/' + (IP ? 'ip' + IP + '/' : '');
  ...
  function Radio_n(object) {
	...
	function startPlugin() {
      ...
      title_prioriry_balanser_descr: {
    	...
    	info_pub_descr: {
    		ru: '<b>KinoPub</b> платный ресурс, просмотр онлайн с тсточника, а так же спортивные ТВ каналы будут доступны после покупки <b>PRO</b> в аккаунте на сайте',
		...
		function add() {
			...
			Lampa.SettingsApi.addParam({
				...
				field: {
					name: '<img src="'+API.replace('api.','')+'/group.jpg"></a>',
					description: 'Для активации <b style="color: #ffd402;">Vip</b> статуса перейдите в телеграм бот <span style="color: #24b4f9;">@modssmy_bot</span><br><a href="https://t.me/+yHKZdjlta5E5MGY6">'
				}
			});
      ...
      Lampa.SettingsApi.addParam({
				...
				field: {
					name: '🆔 5897013793 🔹  💻 Windows NT 10.0 (x64)'
				}
			});
			Lampa.SettingsApi.addParam({
				component: 'settings_modss',
      ...
      Lampa.SettingsApi.addParam({
    	...
    	Lampa.SettingsApi.addParam({
		  ...
		  //Proxy mods
			...
			['VideoCDN', 'HDRezka', 'CDNmovies', 'Collaps'].forEach(function (itm) {
				Lampa.SettingsApi.addParam({
					component: 'mods_proxy',
					param: {
